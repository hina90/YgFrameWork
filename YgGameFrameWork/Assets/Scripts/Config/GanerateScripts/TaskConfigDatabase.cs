...
namespace Tool.Database
{
    ...
    public class TaskConfigDatabase : IDatabase
    {
        public const uint TYPE_ID =15;
        public const string DATA_PATH ="Config/TaskConfig";

        private List<TaskConfigData> m_datas;

        public  TaskConfigDatabase() { }
		...
		private List<TaskConfigData> GetAllData(string[][] m_datas)
		{
			List<TaskConfigData> m_tempList = new List<TaskConfigData>();
			for (int i = 0; i < m_datas.Length; i++)
            {
				TaskConfigData m_tempData = new TaskConfigData();
				...
				if (!int.TryParse(m_datas[i][3].Trim(),out m_tempData.conArgument))
				{
					m_tempData.conArgument=0;
				}
        ...
        public TaskConfigData GetDataByKey(string key)
        {
			return m_datas.Find(temp => temp.taskID == int.Parse(key));
        }

		public List<TaskConfigData> FindAll(Predicate<TaskConfigData> handler = null)
		{
			if (handler == null)
            {
                return m_datas;
